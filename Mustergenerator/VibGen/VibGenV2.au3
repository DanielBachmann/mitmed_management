...
#
#	 Es werden txt dateien erzeugt, deren Inhalt per BLE übertagen
#	 werden können.
#
# ******************************************************************
...
$Form1_1 = GUICreate("MITmed Vibrations-Muster-Generator ", 623, 431, 282, 197)
$btnNew = GUICtrlCreateButton("Neues Muster", 24, 368, 99, 25)
$btnFromList = GUICtrlCreateButton("Liste Einlesen", 24, 400, 99, 25)
...
; Muster aus einer Liste aufbauen, die mit einer EXCEL Vorlage erstellt wurde
; erste Zeile ist Legende und wird ignoriert
; dann folgen Tab-Separierte Felder, jede Zeile beschreibt ein Intervall

Func FromList()
	$fnIn = FileOpenDialog("Vibrator Pattern aus Tabelle", @ScriptDir & "\", "Tabelle (*.csv)", 1)
    If @error Then
		Return
	EndIf
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $fnIn = ' & $fnIn & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	Local $sDrive = "", $sDir = "", $sFileName = "", $sExtension = ""

	_PathSplit($fnIn, $sDrive, $sDir, $sFileName, $sExtension)
;	$fn = $sDrive & "\" & $sDir& "\" & $sFileName & ".txt"
	$fn = $sFileName & ".txt"
	GUICtrlSetData($Label2,$fn)

	$linenum = 2;
	$line = FileReadLine($fnIn,$linenum); // erste Zeile enthält die Legende
	$Fields = StringSplit($line, ";")

	$startTime = 0;
	$index = 0

	; nun die interne Liste aufbauen
	While $Fields[0] = 5
		$Mots = StringSplit($Fields[1], ",");
		$pulseMS = $Fields[2]
		$preMS = $Fields[3]
		$Lpwm = $Fields[4] * 25
		$delayMS = $Fields[5]
		$startTime = $startTime + $delayMS; positive Werte verzögern den Puls, negative lassen ihn früher anfangen
		for $m = 1 to $Mots[0]
		   $mot = $Mots[$m]
			ConsoleWrite($mot & @CRLF)
			if $preMS > 0 then
			    $pattern[$index][0] = $startTime + $delayMS
				$pattern[$index][1] = $mot
				$pattern[$index][2] = 255
				$index = $index + 1
			EndIf
			$pattern[$index][0] = $startTime + $delayMS + $preMS
			$pattern[$index][1] = $mot
			$pattern[$index][2] = $Lpwm
			$index = $index + 1
			$pattern[$index][0] = $startTime + $delayMS + $preMS + $pulseMS
			$pattern[$index][1] = $mot
			$pattern[$index][2] = 0; wieder aus
			$index = $index + 1
		Next
		$startTime = $StartTime + $pulseMS  + $preMS + $delayMS
